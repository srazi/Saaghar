...
bool RegisterationForm::showRegisterForm()
{
	...
	QStringList tmpList = Settings::READ("RegisterationState").toStringList();
	if (tmpList.isEmpty()) return true;
	if (tmpList.at(0) == "NeverASk") return false;
	if (tmpList.size() != 4 || tmpList.at(0) != "AskLater") return true;
...
//	dataHash.value("user_id");
	...
	if (education=="Student") { education = tr("Student"); }
	else if (education=="BSc") { education = tr("BSc (student or graduated)"); }
	else if (education=="MSc") { education = tr("MSc (student or graduated)"); }
	else if (education=="PhD") { education = tr("PhD (student or graduated)"); }
	else if (education=="Other") { education = tr("Other"); }
...
qDebug() << "field111="<<field<<"index="<<ui->comboBoxFieldOfStudy->findText(field);
	if (field=="Mathematics") { field = tr("Mathematics"); }
	else if (field=="Computer") { field = tr("Computer"); }
	else if (field=="Literature") { field = tr("Literature"); }
	else if (field=="Art") { field = tr("Art"); }
	else if (field=="Human Sciences") { field = tr("Human Sciences"); }
	else if (field=="Engineering") { field = tr("Engineering"); }
	else if (field=="Basic Sciences") { field = tr("Basic Sciences"); }
	else if (field=="Other") { field = tr("Other"); }
...
QHash<QString, QString> RegisterationForm::getInfo(const QString &email, const QString &pass, bool showProgress)
{
	...
	if (email.isEmpty()) return registeredUserData;
